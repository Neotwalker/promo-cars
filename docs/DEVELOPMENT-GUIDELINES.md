...
Для landscape-планшетов 821–1100px допускается отдельная более плотная двухколоночная tablet-композиция ключевых секций. Это не возврат к desktop-layout: интерактивы, размеры контролов и сценарии остаются tablet-first.

Для landscape до 820px нужно отдельно уменьшать вертикальный ритм. На 641–820px допустимы локальные двухколоночные композиции Journey и Final CTA, а с ~700px — Configurator и Safety, если сохраняется читаемость. На ≤640px приоритет — короткая первая экранная зона, доступные CTA и сохранение touch-target размеров.
